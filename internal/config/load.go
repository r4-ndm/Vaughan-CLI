...
import (
	...
	"github.com/r4v3n/vaughan-cli/internal/env"
...
func PushPopVaughanEnv() func() {
	found := []string{}
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "VAUGHAN_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "VAUGHAN_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("VAUGHAN_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("VAUGHAN_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
