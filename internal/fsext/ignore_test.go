...
func TestVaughanIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	subVaughanignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".crushignore"), []byte(subVaughanignore), 0o644); err != nil {
